...
// Zod Schema for incident validation with boundaries
const incidentSchema = z.object({
  type: z.enum(['medical', 'security', 'crowd', 'facility']),
  zone: z.string()
    .min(1, 'Please specify the location zone.')
    .max(100, 'Location zone cannot exceed 100 characters.'),
...
export const EmergencyAi: React.FC = () => {
  ...
  const onSubmit = async (data: IncidentFormData) => {
    // Dispatch to store with input sanitization
    addIncident({
      type: data.type,
      zone: sanitizeInputText(data.zone),
      severity: data.severity,
      description: sanitizeInputText(data.description),
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-6xl mx-auto">
      ...
      <div className="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            />
            {errors.zone && <span className="text-[10px] text-rose-450 mt-1 block">{errors.zone.message}</span>}
          </div>
          ...
          <div>
            ...
            />
            {errors.description && <span className="text-[10px] text-rose-450 mt-1 block">{errors.description.message}</span>}
          </div>
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-rose-600 hover:bg-rose-505 text-white font-bold text-xs flex items-center justify-center gap-2 transition-all disabled:opacity-55"
          >
      ...
      <div className="lg:col-span-7 flex flex-col gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex-1 flex flex-col justify-between">
          <div>
            <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2 mb-4">
              <AlertTriangle className="h-5 w-5 text-amber-505" />
              Active Incident Log
            </h3>
                  ...
                  <div key={inc.id} className="p-3 bg-slate-950/40 border border-slate-850 rounded-xl flex items-start justify-between gap-3 text-xs">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-slate-505 font-mono">@{inc.reportedAt}</span>
                      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-950/40 p-3 rounded-xl border border-slate-855 space-y-2">
              ...
              {[
                ...
                <div key={i} className="flex justify-between items-center text-xxs border-b border-slate-855/60 pb-1.5 last:border-0 last:pb-0">
