...
export const AiAnnouncementGenerator: React.FC = () => {
  ...
  const [results, setResults] = useState<{
    ...
    translated: {
      ...
      fr: "⚠️ ATTENTION À TOUS LES SPECTATEURS: Veuillez noter qu'il y a une forte affluence près de la Porte 4. Nous vous recommandons d'utiliser les entrées alternatives comme la Porte 3. Merci de votre coopération.",
  ...
  const handleGenerate = async () => {
    ...
    try {
      const sanitized = sanitizeInputText(inputText).substring(0, 500);
      const prompt = `generate announcement for: "${sanitized}"`;
      ...
      setResults({
        professional: `⚠️ ATTENTION ALL SPECTATORS: Please note operations details regarding ${sanitized}. Stadium operations recommends taking all standard precautions. Direct stewards are nearby to assist.`,
        emergency: `🚨 EMERGENCY ADVISORY: Critical incident alert regarding ${sanitized}. Avoid this area. Immediately follow steward directives.`,
        translated: {
          es: `⚠️ ATENCIÓN: Alerta de operaciones: ${sanitized}. Por favor, siga las instrucciones del personal.`,
          fr: `⚠️ ATTENTION: Alerte d'opérations: ${sanitized}. Veuillez suivre les instructions du personnel.`,
          ar: `⚠️ تنبيه: حالة عمليات بخصوص: ${sanitized}. يرجى اتباع التعليمات المباشرة.`,
          de: `⚠️ ACHTUNG: Betriebsmeldung: ${sanitized}. Bitte den Anweisungen des Personals folgen.`
        },
        sms: `FIFA 2026 Operations: ${sanitized}. Please follow guides. Updates: fifa.sh/ops`,
        push: `🔔 Alert: Operations update regarding ${sanitized}. Check map.`
      });
    } finally {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-6xl mx-auto">
      ...
      <div className="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
        ...
        <div className="space-y-4 my-6">
          <div>
            <label className="text-xxs font-bold text-slate-400 uppercase tracking-wider block mb-1">Enter Raw Situation Briefing</label>
            <textarea
              value={inputText}
        ...
        <div className="p-3 bg-slate-950/40 border border-slate-850 rounded-lg text-xxs text-slate-400 leading-relaxed">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
          ...
          <div className="space-y-4 max-h-[460px] overflow-y-auto pr-1 no-scrollbar">
            ...
            <div className="p-3.5 bg-slate-950/40 border border-slate-850 rounded-xl space-y-2 relative">
              <div className="flex justify-between items-center text-xxs font-bold text-slate-300">
              ...
              <p className="text-xs text-slate-200 leading-relaxed">{results.professional}</p>
            </div>

            {/* 2. Emergency Version */}
            <div className="p-3.5 bg-rose-950/10 border border-rose-500/20 rounded-xl space-y-2 relative">
              <div className="flex justify-between items-center text-xxs font-bold text-rose-400">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="px-2 py-0.5 rounded bg-rose-600 hover:bg-rose-505 text-[9px] font-bold text-slate-105 transition-colors"
                  >
                    Broadcast Crisis
                  </button>
                </div>
              </div>
              <p className="text-xs text-slate-200 leading-relaxed">{results.emergency}</p>
            </div>

            {/* 3. Text Translations Accordion */}
            <div className="p-3.5 bg-slate-950/40 border border-slate-850 rounded-xl space-y-2">
              <span className="text-xxs font-bold text-slate-300 block border-b border-slate-855 pb-2">Gemini Multi-Language Translations</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="p-3 bg-slate-950/40 border border-slate-850 rounded-xl space-y-2 text-xxs">
                <div className="flex justify-between items-center font-bold text-slate-300">
                  ...
                  <button onClick={() => copyToClipboard(results.push, 'push')} className="p-0.5 text-slate-455 hover:text-white">
