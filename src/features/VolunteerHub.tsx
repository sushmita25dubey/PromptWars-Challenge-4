...
export const VolunteerHub: React.FC = () => {
  ...
  const handleQuickReportSubmit = (e: React.FormEvent) => {
    ...
    addIncident({
      ...
      description: `[Volunteer Report]: ${reportDesc}`,
      severity: 'medium',
      zone: reportZone,
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-6xl mx-auto">
              ...
              {!isCheckedIn ? (
                ...
                volunteerTasks.map((task) => (
                  <div key={task.id} className="p-4 bg-slate-950/40 border border-slate-850 rounded-xl space-y-3 hover:border-slate-800 transition-colors">
                    ...
                    <p className="text-xxs sm:text-xs text-slate-350 leading-relaxed">{task.description}</p>
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
          ...
          <p className="text-xxs text-slate-450 mb-4 leading-relaxed">Report stadium damage, spills, medical cases, or lockouts directly to operators.</p>

          <form onSubmit={handleQuickReportSubmit} className="space-y-3">
            <div>
              <label className="text-xxs font-bold text-slate-450 uppercase tracking-wider block mb-1">Location Zone</label>
              <input
                type="text"
                value={reportZone}
            ...
            <div>
              <label className="text-xxs font-bold text-slate-450 uppercase tracking-wider block mb-1">Type</label>
              <select
                value={reportType}
            ...
            <div>
              <label className="text-xxs font-bold text-slate-450 uppercase tracking-wider block mb-1">Details</label>
              <textarea
                value={reportDesc}
