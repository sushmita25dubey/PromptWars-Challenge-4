...
// Reusable Memoized KPI Card Component
...
const KpiCard: React.FC<KpiCardProps> = React.memo(({ label, value, sub, icon: Icon, colorClass }) => {
  return (
    <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl flex items-center justify-between shadow-md">
      <div className="space-y-1">
        ...
        <p className="text-xl font-black text-slate-200 mt-1">{value}</p>
        <span className="text-[9px] text-slate-500 font-medium block">{sub}</span>
      </div>
...
// Reusable Memoized Sector Density Card Component
interface StandDensityCardProps {
  name: string;
  crowdDensity: number;
  currentCount: number;
}

const StandDensityCard: React.FC<StandDensityCardProps> = React.memo(({ name, crowdDensity, currentCount }) => {
  const isOvercrowded = crowdDensity > 80;
  return (
    <div className="p-3 bg-slate-950/40 border border-slate-850 rounded-xl space-y-2">
      <div className="flex justify-between items-center">
        <span className="text-xxs font-bold text-slate-200">{name}</span>
      ...
      <div className="flex justify-between items-center text-[10px] text-slate-500">
        ...
        <span>{currentCount.toLocaleString()} visitors</span>
      </div>
    </div>
  );
});
StandDensityCard.displayName = 'StandDensityCard';
...
export const DashboardPage: React.FC = () => {
  ...
  const liveMatch = useMemo(() => {
    return matchTelemetry.find(m => m.status === 'live') || matchTelemetry[0];
  }, [matchTelemetry]);
  ...
  const crowdStats = useMemo(() => {
    ...
    return { totalCapacity, totalCount, avgDensity };
  }, [stadiumZones]);

  // Status indicators helpers
  const activeAlerts = useMemo(() => alerts.filter(a => a.status === 'active'), [alerts]);
  const activeTasks = useMemo(() => volunteerTasks.filter(t => t.status !== 'completed'), [volunteerTasks]);
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard
          ...
          value={`${crowdStats.avgDensity}%`}
          sub={`${crowdStats.totalCount.toLocaleString()} / ${crowdStats.totalCapacity.toLocaleString()}`}
          icon={Users}
          colorClass="text-fifa-gold bg-fifa-gold/10"
        />
        <KpiCard
          label="Active Alerts"
          value={activeAlerts.length.toString()}
          sub={`${alerts.length - activeAlerts.length} Resolved`}
          icon={ShieldAlert}
          colorClass={activeAlerts.length > 0 ? 'text-rose-450 bg-rose-505/10 animate-pulse' : 'text-slate-400 bg-slate-800/60'}
        />
        <KpiCard
          label="Volunteer Staff"
          value="420"
          sub={`${activeTasks.length} tasks dispatching`}
          icon={Award}
          colorClass="text-blue-400 bg-blue-500/10"
        />
        <KpiCard
          label="Eco Offset"
          value={`${sustainability.environmentalScore}%`}
          sub={`${sustainability.wasteRecycledKg}kg recycled`}
          icon={Leaf}
          colorClass="text-emerald-450 bg-emerald-505/10"
        />
      </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl relative overflow-hidden">
            ...
            <div className="flex items-center justify-between">
              <span className="text-[10px] bg-rose-600/10 text-rose-500 border border-rose-505/20 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider animate-pulse flex items-center gap-1">
          ...
          <div className="bg-slate-900 border border-slate-855 rounded-2xl p-5 shadow-xl">
          ...
          <div className="bg-slate-900 border border-slate-855 rounded-2xl p-5 shadow-xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                const Icon = res.icon;
                return (
                  <div key={i} className="p-3 bg-slate-950/40 border border-slate-855 rounded-xl space-y-2 flex flex-col justify-between">
