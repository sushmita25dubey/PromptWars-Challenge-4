...
export const EmergencyButton: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Auto-focus the close button or first interactive element inside the dialog
    const focusable = modalRef.current?.querySelectorAll('button');
    if (focusable && focusable.length > 0) {
      // Index 0 is the close 'X' button
      setTimeout(() => {
        focusable[0].focus();
      }, 50);
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm animate-fade-in">
          ...
          >
            ...
            <div className="p-5 bg-rose-950/30 border-b border-rose-900/30 flex items-center justify-between">
              ...
              <button
                onClick={resetSOS}
                className="p-1 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-rose-500"
            ...
            <div className="p-6">
              {!sosSent ? (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      ...
                      className="p-5 rounded-xl bg-slate-800/60 hover:bg-rose-950/20 border border-slate-700 hover:border-rose-500/50 flex flex-col items-center justify-center gap-3 text-center transition-all group animate-fade-in focus:outline-none focus:ring-2 focus:ring-rose-500"
                    >
                    ...
                    <button
                      ...
                      className="p-5 rounded-xl bg-slate-800/60 hover:bg-rose-950/20 border border-slate-700 hover:border-rose-500/50 flex flex-col items-center justify-center gap-3 text-center transition-all group animate-fade-in focus:outline-none focus:ring-2 focus:ring-amber-500"
                    >
                ...
                <div className="text-center py-4">
                  ...
                  <button
                    onClick={resetSOS}
                    className="w-full py-2.5 rounded-xl bg-slate-800 hover:bg-slate-750 text-slate-200 font-semibold transition-colors border border-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500"
                  >
                    Close & Keep Active
                  </button>
                </div>
